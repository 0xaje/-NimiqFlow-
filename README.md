...
- **AI Smart Invoice Generator**: Exclusive AI feature for drafting and itemizing invoices with real-time CoinGecko exchange calculations, scan-to-pay QR codes, and PDF export (`window.print()`).
- **Multilingual Support (i18n)**: Automatic locale detection with one-tap switcher for English (`en`), German (`de`), and Spanish (`es`).
- **Developer Settings**: Network toggle between `TestAlbatross` and `MainAlbatross` with real-time RPC node status, consensus tracking, and block height queries.
